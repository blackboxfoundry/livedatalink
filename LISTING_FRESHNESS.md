...
| mcp.so (primary and translations) | Shows 62/14, 100 free queries, $0.01 overage, stdio, and no tools. | Update the claimed publisher submission; request a manual correction if the dashboard does not persist. One primary correction should feed locale mirrors. | Listing claimed. Dashboard correction save failed silently and reverted after reload; support ticket “Claimed LiveDataLink listing cannot save corrections” submitted 2026-08-16. Pending mcp.so response/manual correction. | Weekly until corrected, then monthly |
